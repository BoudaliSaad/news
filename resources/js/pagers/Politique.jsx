...
function IndexPage() {
    const [newsPlicy, setnewsPolicy] = useState([])
    useEffect(() => {
        async function fetchData() {
            try {
                const fetchnews = await fetch('https://newsapi.org/v2/top-headlines?apiKey=ec32aeafdb5a4d2eb4087e4822970d5d&q=policy');
                const data = await fetchnews.json();

                setnewsPolicy(data.articles)
            } catch (error) {
                console.log(error);
            }
        }
